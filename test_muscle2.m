...
if ismember('phase',doplot)
    ...
    for i = 1:4
        ...
        addplot(hax(i,1), phasedata(j).t, phasedata(j).Pc, 'r-', 'LineWidth',2);
        addplot(hax(i,2), phasedata(j).t, phasedata(j).L,'k-');
        axis(hax(i,2),'tight');
    ...
    fxx = cat(4, phasedata.fx);
    sgn1 = sign(fxx(1,1,1,:));
    fxx(:,:,1,:) = bsxfun(@times, fxx(:,:,1,:), sgn1);

    fexp = cat(2,phasedata.fexp);
    ...
    i = 1;
    subplot(2,2,1);
    plot(phasedata(i).t, fxx(:,:,1,i), 'LineWidth',2);
    xlabel('Time (s)');
    ...
    k = [a:length(t) 1:a-1]';
    dec = exp(phasedata(i).fexp(1)*phasedata(i).t);
    dec1 = zeros(size(dec));
    dec1(k) = dec;

    subplot(2,2,2);
    plot(phasedata(i).t, dec1,'k-', 'LineWidth',2);
    xlabel('Time (s)');

    subplot(2,4,6:7);
    plot(phasedata(i).t, bsxfun(@times, fxx(:,:,1,i), dec1), 'LineWidth',2);
    xlabel('Time (s)');
